...
export default function CatchTable({
  ...
  if (catches.length === 0) {
    return (
      <div className="bg-white rounded-2xl border border-adventure-border shadow-card p-12 flex flex-col items-center gap-4 text-center">
        <div className="w-16 h-16 rounded-2xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #2D6A4F, #3D9068)' }}>
        ...
        <p className="text-sm font-sans text-earth-400 max-w-xs">
          No catches match your current filters. Try adjusting the search or water type, or log a new catch from today&apos;s trip.
        </p>
      </div>
    );
  }
  ...
  const cols: Array<{ key: keyof CatchEntry; label: string; sortable: boolean }> = [
    ...
    { key: 'bait', label: 'Bait', sortable: false },
            ...
            {catches.map((catch_, idx) => (
              <tr
                ...
                className={`
                  ...
                  ${selectedIds.has(catch_.id) ? 'bg-primary-50' : idx % 2 === 0 ? 'bg-white' : 'bg-earth-50/30'}
              ...
              >
                <td className="pl-4 pr-2 py-3">
                  <input
                    type="checkbox"
                    checked={selectedIds.has(catch_.id)}
                    onChange={() => onToggleSelect(catch_.id)}
                    className="w-4 h-4 rounded border-adventure-border accent-primary-600"
                    aria-label={`Select ${catch_.species} catch`}
                  />
                </td>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <span className="text-lg">{catch_.emoji}</span>
                ...
                <td className="px-4 py-3 text-sm font-sans text-primary-700 max-w-[140px]">
                  <span className="truncate block">{catch_.location}</span>
                </td>

                {/* Water type */}
                <td className="px-4 py-3">
                  <span className={`inline-flex items-center text-xs font-sans font-semibold px-2 py-0.5 rounded-full border ${waterTypeBadge[catch_.waterType] || 'bg-gray-100 text-gray-600 border-gray-200'}`}>
                    {catch_.waterType}
                  </span>
                </td>

                {/* Weather */}
                <td className="px-4 py-3 text-sm font-sans text-earth-500 whitespace-nowrap">
                  <span className="flex items-center gap-1">
                    <span>{weatherIcon[catch_.weather] || '🌤️'}</span>
                    <span className="hidden xl:inline">{catch_.weather}</span>
                  </span>
                </td>

                {/* Bait */}
                <td className="px-4 py-3 text-sm font-sans text-earth-500 max-w-[120px]">
                  <span className="truncate block">{catch_.bait}</span>
                </td>

                {/* Notes */}
                <td className="px-4 py-3 text-xs font-sans text-earth-400 max-w-[160px]">
                  <span className="truncate block">{catch_.notes}</span>
                </td>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                    ...
                    {deleteConfirmId === catch_.id ? (
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() => confirmDelete(catch_.id)}
                      ...
                      <div className="relative group/del">
                        <button
                          onClick={() => setDeleteConfirmId(catch_.id)}
                          className="p-1.5 rounded-lg hover:bg-red-50 text-earth-400 hover:text-red-500 transition-colors"
                          aria-label={`Delete ${catch_.species} catch — this cannot be undone`}
                        >
