...
import { allCards, rarityConfig, CardRarity, FishingCard } from '@/app/card-collection/data/cardData';
...
export default function CollectionBookClient() {
  ...
  const [cards, setCards] = useState<FishingCard[]>(allCards);
  ...
  useEffect(() => {
    ...
    supabase
      .from('user_cards')
      .select('card_id, collected_at, cards(card_number)')
      .eq('user_id', user.id)
      .then(({ data }) => {
        if (!data) { setLoading(false); return; }
        const collectedMap = new Map<number, string>();
        data.forEach((uc: Record<string, unknown>) => {
          const cardData = uc.cards as { card_number: number } | null;
          if (cardData) collectedMap.set(cardData.card_number, uc.collected_at as string);
        });
        const merged = allCards.map(c => ({
          ...c,
          collected: collectedMap.has(c.cardNumber),
          collectedDate: collectedMap.get(c.cardNumber)
            ? new Date(collectedMap.get(c.cardNumber)!).toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: 'numeric' })
            : undefined,
        }));
        setCards(merged);
        setLoading(false);
  ...
  return (
    <>
      {/* Book cover header */}
      <div className="relative overflow-hidden rounded-3xl p-6 lg:p-8 mb-6" style={{ background: 'linear-gradient(135deg, #091408 0%, #1A3D28 50%, #2D6A4F 100%)' }}>
        ...
        />
        <div className="absolute top-0 right-0 w-80 h-80 rounded-full opacity-10" style={{ background: 'radial-gradient(circle, #ff751f, transparent)', transform: 'translate(30%, -30%)' }} />
